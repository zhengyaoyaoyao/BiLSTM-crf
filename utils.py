...
def get_vacob(config):
    df = pd.read_csv(config.VOCAB_PATH,names=['word','id'])
    return list(df['word']),dict(df.values)     # df['word']即word的列表，values就是word和id都包含了。

def get_label(config):
    df = pd.read_csv(config.LABEL_PATH,names=['label','id'])
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index) -> T_co:
        ...
        return input,target  # 所以这里的input 和target就已经是词表里面的id了。所以如果后续使用bert，所以config那里配置一个tokenizer。两个都需要，所以再另一个项目用了一个field、
